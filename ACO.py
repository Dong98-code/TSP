...
while iter < itermax:
    ...
    if numant <= numcity:#城市数比蚂蚁数多
        ...
        #np.random.permutation 随机排列一个序列，返回一个排列的序列，即对城市数随机排一个序列
    else: #蚂蚁数比城市数多，需要补足
    ...
    for i in range(numant):#对每一个蚂蚁计算其相关指标
...
print('/n')
print("最短路径和为：%s" % (lengthbest[-1]))
end = time.perf_counter()
print("程序的运行时间是：%s" % (end - start))
# 做出平均路径长度和最优路径长度
fig,axes = plt.subplots(nrows=2,ncols=1,figsize=(12,10))
axes[0].plot(lengthaver,'r',marker = u'')
axes[0].set_title('Average Length')
axes[0].set_xlabel(u'iteration')

axes[1].plot(lengthbest,'r',marker = u'')
axes[1].set_title('Best Length')
axes[1].set_xlabel(u'iteration')
fig.savefig('Average_Best.png',dpi=1000,bbox_inches='tight')
plt.close()
...
plt.plot([coordinates[int(bestpath[0])][0],coordinates[n][0]],[coordinates[int(bestpath[0])][1],
                                                                   coordinates[n][1]],'b')
ax=plt.gca()
ax.set_title("best path(ACO)")
ax.set_xlabel('X_axis')
ax.set_ylabel('Y_axis')

# plt.savefig('Best Path_Ant.png',dpi=1000,bbox_inches='tight')
plt.savefig('best path(ACO).png',dpi=1000)
plt.close()
